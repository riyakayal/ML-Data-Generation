...
def valid_molecule(mol):
    ...
    # Check for neutral molecule
    for atom in mol.GetAtoms():
        if atom.GetFormalCharge() != 0:
            return False, "charged molecule"

    # Check for single fragment
    frags = Chem.GetMolFrags(mol, asMols=True)
    if len(frags) > 1:
        return False, "multi-fragment molecule"
...
print("Streaming SDF and extracting molecules (neutral, single-fragment)...")
