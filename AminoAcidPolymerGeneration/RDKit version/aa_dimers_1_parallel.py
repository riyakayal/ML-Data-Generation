...
# CPU Parallelisation enabled version.
...
# ===============================
# Single Job Function (Parallel Unit)
# ===============================

def generate_single_pair(args):

    name1, smi1, name2, smi2, output_dir = args
    ...
    try:
        ...
        return f"Generated {name1}-{name2}"

    except Exception as e:
        return f"Failed {name1}-{name2}: {e}"
...
def generate_all(output_dir="dimers_1_parallel"):
    ...
    for name1, smi1 in AA_SMILES.items():
        for name2, smi2 in AA_SMILES.items():
            tasks.append((name1, smi1, name2, smi2, output_dir))

    print(f"Total dimers: {len(tasks)}")
    print(f"Using {N_WORKERS} CPU cores")

    with Pool(processes=N_WORKERS) as pool:
        results = pool.map(generate_single_pair, tasks)
